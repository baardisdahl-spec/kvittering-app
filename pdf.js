...
// Robust file sharing/download across platforms
async function shareOrDownload(blob, filename, mimeType) {
  const isIOS = /iPhone|iPad|iPod/.test(navigator.userAgent) || 
                (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
  const inPWA = isPWAMode();

  // Try Web Share API first — best UX on iOS/Android when available
  try {
  ...
  if (isIOS) {
    // iOS Safari/PWA: show PDF inline with share button
    // Convert blob to data URL (most reliable for iOS)
    ...
    // In PWA mode, we can't open new windows — show inline overlay instead
    showPDFOverlay(dataUrl, filename, blob, mimeType);
    return;
  }
...
// Inline PDF viewer with share button for iOS
function showPDFOverlay(dataUrl, filename, blob, mimeType) {
  const overlay = document.createElement('div');
  overlay.style.cssText = `
    position: fixed; inset: 0; background: #f2f2f7; z-index: 10000;
    display: flex; flex-direction: column;
  `;
  overlay.innerHTML = `
    <div style="padding: 12px 16px; padding-top: max(12px, env(safe-area-inset-top));
                background: white; border-bottom: 0.5px solid #d1d1d6;
                display: flex; justify-content: space-between; align-items: center;
                font-family: -apple-system, sans-serif;">
      <button id="pdf-close" style="background: none; border: none; color: #007aff; 
              font-size: 16px; cursor: pointer; padding: 8px;">Lukk</button>
      <strong style="font-size: 15px; max-width: 60%; overflow: hidden; 
              text-overflow: ellipsis; white-space: nowrap;">${filename}</strong>
      <button id="pdf-share" style="background: none; border: none; color: #007aff; 
              font-size: 16px; cursor: pointer; padding: 8px; font-weight: 600;">Del</button>
    </div>
    <iframe src="${dataUrl}" style="flex: 1; border: none; background: white;"></iframe>
  `;
  document.body.appendChild(overlay);

  document.getElementById('pdf-close').onclick = () => overlay.remove();
  
  document.getElementById('pdf-share').onclick = async () => {
    try {
      const file = new File([blob], filename, { type: mimeType });
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
    ...
    // Fallback: open data URL in new tab so user can share from there
